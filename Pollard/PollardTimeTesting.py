...
########## Pollard Rho Testing ##########
def PollardRhoBase(n, rand, thread):
    start = timer()
    i = 1
    ...
    while True:
        ...
        d = gmpy2.gcd(y-x, n)
        if d != 1 and d != n:
            print("Pollard Base p thread " + str(thread) + ": " + str(d))
            return timer() - start
        if i == k:
            y = x
            k = k * 2

def PollardRhoMPZ(n, rand, thread):
    start = timer()
    i = mpz(1)
    x = gmpy2.mpz_random(gmpy2.random_state(rand), n-1)
    y = x
    ...
    while True:
        ...
        if d != 1 and d != n:
            print("Pollard MPZ p thread " + str(thread) + ": " + str(d))
            return timer() - start
        if i == k:
            y = x
            k = gmpy2.mul(mpz(k), 2)
...
def PollardP1Base(n, thread):
    ...
    while True:
        ...
        g = gmpy2.gcd((a-1), n)
        if g != 1 and g != n:
            print("P1 Base p thread " + str(thread) + ": " + str(g))
            return timer() - start
        B += 1

def PollardP1MPZ(n, thread):
    start = timer()
    a = mpz(2)
    B = mpz(2)
    while True:
        ...
        if g != 1 and g != n:
            print("P1 MPZ p thread " + str(thread) + ": " + str(g))
            return timer() - start
        B = gmpy2.add(B, 1)
...
def PollardRhoBaseMulti(num_processes, size):
    ...
    for i in range(1, processes+1):
        ...
        print("Pollard Base n thread " + str(i) + ": " + str(privkey.n))
        t1 = t1 + PollardRhoBase(privkey.n, time_var, i)
...
def PollardRhoMPZMulti(num_processes, size):
    ...
    for i in range(1, processes+1):
        (pubkey, privkey) = rsa.newkeys(size)
        print("Pollard MPZ n thread " + str(i) + ": " + str(privkey.n))
        t1 = t1 + PollardRhoMPZ(privkey.n, time_var, i)
        time_var = time_var + 10000
...
def PollardP1BaseMulti(num_processes, size):
    ...
    for i in range(1, processes+1):
        (pubkey, privkey) = rsa.newkeys(size)
        print("P1 Base n thread " + str(i) + ": " + str(privkey.n))
        t1 = t1 + PollardP1Base(privkey.n, i)
        time_var = time_var + 10000
...
def PollardP1MPZMulti(num_processes, size):
    ...
    for i in range(1, processes+1):
        (pubkey, privkey) = rsa.newkeys(size)
        print("P1 MPZ n thread " + str(i) + ": " + str(privkey.n))
        t1 = t1 + PollardP1MPZ(privkey.n, i)
        time_var = time_var + 10000
